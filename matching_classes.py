...
class LogicNN(object):
    ...
    def calc_distr(self):
        ...
        for i,rule in enumerate(self.rules):
            distr = rule.distr(self.C * self.rule_lambda[i])
            distr_all += distr
    ...
    def cost(self):
        ...
        cost = 0.5 * L_sup + 0.5 * L_p_q + self.mu_param*L_sqr
        return cost, L_sup, L_p_q, L_sqr, p_m12, p_m13, q_m12, q_m13
    ...
    def dropout_cost(self):
        ...
        L_sqr = self.network.sqr
        dropout_cost = 0.5 * L_sup + 0.5 * L_p_q + self.mu_param*L_sqr
        return dropout_cost
